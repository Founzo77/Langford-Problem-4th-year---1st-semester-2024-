	...
	namespace Aruka
	{
		namespace OpenMP
		{
			template <uint8_t C, uint8_t F,
				...
				std::size_t NB_BYTES = static_cast<std::size_t>(2e10),
				std::size_t ROOT_DEPTH = 4,
				std::size_t NB_THREADS_PER_BLOCK = 1>
			...
			private:
				...
				std::vector<m_Dispatcher> dispatchers;
			...
			public:
				...
				static std::size_t getNbThreadsBlock() noexcept
				{
					return omp_get_max_threads() / NB_THREADS_PER_BLOCK;
				}

			public:
				HybridStaticResolver() :
					workers(), resolverTracker(), dispatchers()
				{
				...
				void resolve(std::vector<m_Solution>& solutions)
				{
					...
					std::vector<m_Dispatcher>& dispatchers = this->dispatchers;

					std::size_t nbThreadsBlock = getNbThreadsBlock();

					for (std::size_t i = 0; i < nbThreadsBlock; i++)
					{
						dispatchers.push_back({ getDispatcherSizeTasksBlock() });
						for (std::size_t j = i * NB_THREADS_PER_BLOCK;
							j < (i + 1) * NB_THREADS_PER_BLOCK && 
							j < (size_t)omp_get_max_threads(); j++)
						{
							dispatchers[i].registerWorker(j);
						}
					}

					for (std::size_t i = 0; i < (size_t)omp_get_max_threads(); i++)
					{
						workers.push_back({ i, getWorkersSizeTasksBlock() });
					}
					...
					std::vector<m_Task> _treeTasks;
					std::vector<m_Task>& treeTasks = _treeTasks;
					resolveRoot(rootTask, treeTasks);

					std::cout << "nb trees: ";
					std::cout << treeTasks.size() << std::endl;

					std::cout << "nb threads block : " << nbThreadsBlock << std::endl;
					
					#pragma omp parallel for num_threads((int)nbThreadsBlock) shared(treeTasks, workers, dispatchers) schedule(static, 1)
					for (int i = 0; i < (int)treeTasks.size(); i++)
					{
						resolveTree(treeTasks[i], workers, dispatchers[omp_get_thread_num()]);
					}
			...
			private:
				...
				void resolveTree(m_Task& rootTask, std::vector<m_Worker>& workers,
					m_Dispatcher& dispatcher)
				{
					std::size_t globalId = (std::size_t)omp_get_thread_num() * NB_THREADS_PER_BLOCK;
					
					dispatcher.addRootTask(rootTask);

					#pragma omp parallel num_threads(NB_THREADS_PER_BLOCK) shared(dispatcher, workers, globalId, rootTask)
					{
						std::size_t idDispatcher = (std::size_t)omp_get_thread_num();
						std::size_t id = globalId + idDispatcher;
						...
						while (dispatcher.getNbTasks() != 0)
						{
							...
							auto iterator(dispatcher.getIterator(idDispatcher, nbThreads));

							workers[id].getWorkerTracker().taskStarted();

							for (std::size_t i = idDispatcher; i < totalTasks; i += nbThreads)
							{
							...
							#pragma omp single
							{
								...
								{
									std::size_t j = globalId + i;

									workers[j].getWorkerTracker().taskAddedStart();
									workers[j].getWorkerTracker().
										tasksAdded(workers[j].getD().size());

									dispatcher.swapD(i, workers[j].getD());
								}
							}
							#pragma omp barrier
						}
